...
int main (int argc, char *argv[]) {
	...
	*/
	...
	if(rank==0){
	    ...
	    MPI_Scatter(tmp, num_per_node, MPI_INT, node_arr, num_per_node, MPI_INT, 0, MPI_COMM_WORLD);
		comm_end = MPI_Wtime();
	...
	if(rank==0){
		...
		comm_start = MPI_Wtime();
 		MPI_Gather(node_arr, num_per_node, MPI_INT, tmp, num_per_node, MPI_INT, 0, MPI_COMM_WORLD);
		comm_end = MPI_Wtime();
		comm_total += (comm_end - comm_start);
